...
public class FileInout extends PlayerIn {
	...
	public class WriteFileOut implements Runnable {
		...
		public void run() {
			while (running) {
				...
				// log.info(messagesReceived + "/" + messagesSent
				// + " received/sent");
			}
	...
	public class Tailerhandler extends TailerListenerAdapter {
		public void handle(String line) {
			System.out.println("Read in check:: " + line);
			try {
				String id = line.substring(0,line.indexOf(' '));
				String content = line.substring(line.indexOf(' ') + 1);
				messagesIn.offer(new Message(id,content));
				//System.out.println(messagesIn.peekFirst().toString());
				messagesReceived++;
			} catch (StringIndexOutOfBoundsException e) {
				System.out.println("Error in read in...");
			}
			finally {
			}
